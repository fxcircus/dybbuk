...
- Parameter count: 19, in Push 3 page order (Roy, 2026-09-10: the most
  ...
  read aliases the way the hardware's does. Full (Replace / Hold) was removed on
...
- Every parameter is automatable and the stepped ones (Mode, Direction,
  Steps, Pitch, the switches) report themselves discrete, so a host maps
  them to their steps rather than a ramp; `ProcessorTest ordering` asserts
  both. JUCE's integer parameter does not call itself discrete, hence the
  `DiscreteInt` subclass in Parameters.cpp
...
5. **The dice no longer rolls Threshold or Blend.** Both are set to the
